...
@section('content')
    <div id="page-content">
        <form action="{{route('merchant.config-odds-diff')}}" method="post" id="odds-diff-form">
            {{csrf_field()}}
                        ...
                        @foreach($games as $game)
                            ...
                            <table border="0" cellspacing="" cellpadding="0" class="tableRebate">
                                ...
                                @foreach($game->options as $option)
                                    <tr class="rebateLine">
                                        <td>{{$option->title}}</td>
                                        <td><input type="text" class="inputFocus" isNumber="true" maxlength="6" name="option[{{$game->id}}][{{$option->id}}][diff_b]" value="{{$option->diff_b}}"></td>
                                        <td><input type="text" class="inputFocus" isNumber="true" maxlength="6" name="option[{{$game->id}}][{{$option->id}}][diff_c]" value="{{$option->diff_c}}"></td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
